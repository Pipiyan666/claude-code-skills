...
final class CaptureViewModel {
    ...
    @MainActor func startRecording() {
        ...
        Task { @MainActor in
            do {
                try await self.doStartRecordingAsync()
            } catch {
                print("[Voice] 失败: \(error)")
    ...
    @MainActor private func doStartRecordingAsync() async throws {
        // 1. 麦克风权限
        let micOK = await AVAudioApplication.requestRecordPermission()
        guard micOK else { throw VoiceRecordError.micDenied }
        print("[Voice] mic OK")
        ...
        guard speechOK else { throw VoiceRecordError.speechDenied }
        print("[Voice] speech OK")

        // 3. 初始化识别器
        guard let recognizer = SFSpeechRecognizer(locale: Locale(identifier: "zh-CN")),
              recognizer.isAvailable else { throw VoiceRecordError.unavailable }
...
enum VoiceRecordError: LocalizedError {
    case micDenied, speechDenied, unavailable
    var errorDescription: String? {
        switch self {
        case .micDenied: return "请在设置 > 灵感胶囊 > 麦克风 里开启权限"
        case .speechDenied: return "请在设置 > 灵感胶囊 > 语音识别 里开启权限"
        case .unavailable: return "中文语音识别不可用"
        }
    }
}
